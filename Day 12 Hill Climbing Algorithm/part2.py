...
with open('input.txt') as f:
    ...
    for m, row in enumerate(matrix):
        for n, elem in enumerate(row):
            if matrix[m][n] == 'S' or matrix[m][n] == 'a': start_postions.append(cnt)
            ...
            graph[cnt] = next
            cnt += 1
    shortest = len(shortest_path(graph, start_postions[0], E))
    for i in range(1,len(start_postions)):
        path = len(shortest_path(graph, start_postions[i], E)) - 1
        if path == -1: continue
